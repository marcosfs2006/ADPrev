#' Extrai dados do Plano Benefício do DRAA da API do CADPREV
#'
#' Função para a obtenção de dados relativos ao Plano Benefício do 
...
#' ao ponto de acesso \code{DRAA_PLANO_BENEFICIO} recomendamos utilizar os 
...
#'  para a consulta ao plano_beneficio do DRAA.  
#' @return Um \code{data frame} contendo os dados requisitados.
#' @examples
#' \dontrun{ 
#' # Obtém dados do Plano Benefício do DRAA feito pelos RPPS do RJ
#' draa_plano_beneficio_RJ <- get_draa_plano_beneficio(sg_uf="RJ") 
#' 
#' # Obtém dados do Plano Benefício do DRAA feito pelo RPPS de Quatis - RJ
#' draa_plano_beneficio_QuatisRJ <- get_draa_plano_beneficio(nr_cnpj_entidade = "39560008000148")
#' }
#' @export
get_draa_plano_beneficio <- function(...){
  ...
  while(continuar){
    ...
    # Acessando API:
    draa_plano_beneficio <- httr::GET("https://apicadprev.trabalho.gov.br/DRAA_PLANO_BENEFICIO", 
    ...
    draa_plano_beneficio_json <- jsonlite::fromJSON(httr::content(draa_plano_beneficio, as = "text", encoding = "UTF-8"))
    
    # Empilhando dados (o padrao e 5000):
    dados_draa_plano_beneficio <- dplyr::bind_rows(dados_draa_plano_beneficio, draa_plano_beneficio_json[["data"]])
    
    # Se alcancar o limite (5000), vai continuar a busca:
    continuar <- draa_plano_beneficio_json[["count"]] == draa_plano_beneficio_json[["limit"]]
    
    # Avança o offset para a proxima pagina:
    pagina <- pagina + draa_plano_beneficio_json[["limit"]]
