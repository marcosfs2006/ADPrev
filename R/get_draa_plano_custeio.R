#' Extrai dados do Plano de Custeio do DRAA da API do CADPREV
#'
#' Função para a obtenção de dados relativos ao Plano de Custeio do 
...
#' ao ponto de acesso \code{DRAA_PLANO_CUSTEIO} recomendamos utilizar os 
...
#'  para a consulta ao plano_custeio do DRAA.  
#' @return Um \code{data frame} contendo os dados requisitados.
#' @examples
#' \dontrun{ 
#' # Obtém dados do Plano de Custeio do DRAA feito pelos RPPS do RJ
#' draa_plano_custeio_RJ <- get_draa_plano_custeio(sg_uf="RJ") 
#' 
#' # Obtém dados do Plano de Custeio do DRAA feito pelo RPPS de Quatis - RJ
#' draa_plano_custeio_QuatisRJ <- get_draa_plano_custeio(nr_cnpj_entidade = "39560008000148")
#' }
#' @export
get_draa_plano_custeio <- function(...){
  
  consulta <- list(...) # Repassa parametros a api
  pagina <- 0
  dados_draa_plano_custeio <- data.frame()
  continuar <- TRUE
  
  while(continuar){
    
    # Acessando API:
    draa_plano_custeio <- httr::GET("https://apicadprev.trabalho.gov.br/DRAA_PLANO_CUSTEIO", 
                                        query = c(consulta, list(offset = pagina)))
    
    # Mensagem se o site estiver fora do ar ou der erro:
    httr::stop_for_status(draa_plano_custeio, task = "Connect to the server! Try again later.")
    
    # Convertendo dados em lista:
    draa_plano_custeio_json <- jsonlite::fromJSON(httr::content(draa_plano_custeio, as = "text", encoding = "UTF-8"))
    
    # Empilhando dados (o padrao e 5000):
    dados_draa_plano_custeio <- dplyr::bind_rows(dados_draa_plano_custeio, draa_plano_custeio_json[["data"]])
    
    # Se alcancar o limite (5000), vai continuar a busca:
    continuar <- draa_plano_custeio_json[["count"]] == draa_plano_custeio_json[["limit"]]
    
    # Avança o offset para a proxima pagina:
    pagina <- pagina + draa_plano_custeio_json[["limit"]]
    
    Sys.sleep(1)
  }
  
  return(dados_draa_plano_custeio)
}
