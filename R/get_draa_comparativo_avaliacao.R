#' Extrai dados do Comparativo Avaliação do DRAA da API do CADPREV
#'
#' Função para a obtenção de dados relativos ao Comparativo Avaliação do 
...
#' ao ponto de acesso \code{DRAA_COMPARATIVO_AVALIACAO} recomendamos utilizar os 
...
#'  para a consulta ao comparativo_avaliacao do DRAA.  
#' @return Um \code{data frame} contendo os dados requisitados.
#' @examples
#' \dontrun{ 
#' # Obtém dados do Comparativo Avaliação do DRAA feito pelos RPPS do RJ
#' draa_comparativo_avaliacao_RJ <- get_draa_comparativo_avaliacao(sg_uf="RJ") 
#' 
#' # Obtém dados do Comparativo Avaliação do DRAA feito pelo RPPS de Quatis - RJ
#' draa_comparativo_avaliacao_QuatisRJ <- get_draa_comparativo_avaliacao(nr_cnpj_entidade = "39560008000148")
#' }
#' @export
get_draa_comparativo_avaliacao <- function(...){
  
  consulta <- list(...) # Repassa parametros a api
  pagina <- 0
  dados_draa_comparativo_avaliacao <- data.frame()
  continuar <- TRUE
  
  while(continuar){
    
    # Acessando API:
    draa_comparativo_avaliacao <- httr::GET("https://apicadprev.trabalho.gov.br/DRAA_COMPARATIVO_AVALIACAO", 
                                   query = c(consulta, list(offset = pagina)))
    
    # Mensagem se o site estiver fora do ar ou der erro:
    httr::stop_for_status(draa_comparativo_avaliacao, task = "Connect to the server! Try again later.")
    
    # Convertendo dados em lista:
    draa_comparativo_avaliacao_json <- jsonlite::fromJSON(httr::content(draa_comparativo_avaliacao, as = "text", encoding = "UTF-8"))
    
    # Empilhando dados (o padrao e 5000):
    dados_draa_comparativo_avaliacao <- dplyr::bind_rows(dados_draa_comparativo_avaliacao, draa_comparativo_avaliacao_json[["data"]])
    
    # Se alcancar o limite (5000), vai continuar a busca:
    continuar <- draa_comparativo_avaliacao_json[["count"]] == draa_comparativo_avaliacao_json[["limit"]]
    
    # Avança o offset para a proxima pagina:
    pagina <- pagina + draa_comparativo_avaliacao_json[["limit"]]
    
    Sys.sleep(1)
  }
  
  return(dados_draa_comparativo_avaliacao)
}
