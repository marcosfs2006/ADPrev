#' Extrai dados do Parecer Atuarial do DRAA da API do CADPREV
#'
#' Função para a obtenção de dados relativos ao Parecer Atuarial do 
...
#' ao ponto de acesso \code{DRAA_PARECER_ATUARIAL} recomendamos utilizar os 
...
#'  para a consulta ao parecer_atuarial do DRAA.  
#' @return Um \code{data frame} contendo os dados requisitados.
#' @examples
#' \dontrun{ 
#' # Obtém dados do Parecer Atuarial do DRAA feito pelos RPPS do RJ
#' draa_parecer_atuarial_RJ <- get_draa_parecer_atuarial(sg_uf="RJ") 
#' 
#' # Obtém dados do Parecer Atuarial do DRAA feito pelo RPPS de Quatis - RJ
#' draa_parecer_atuarial_QuatisRJ <- get_draa_parecer_atuarial(nr_cnpj_entidade = "39560008000148")
#' }
#' @export
get_draa_parecer_atuarial <- function(...){
  
  consulta <- list(...) # Repassa parametros a api
  pagina <- 0
  dados_draa_parecer_atuarial <- data.frame()
  continuar <- TRUE
  
  while(continuar){
    
    # Acessando API:
    draa_parecer_atuarial <- httr::GET("https://apicadprev.trabalho.gov.br/DRAA_PARECER_ATUARIAL", 
                                         query = c(consulta, list(offset = pagina)))
    
    # Mensagem se o site estiver fora do ar ou der erro:
    httr::stop_for_status(draa_parecer_atuarial, task = "Connect to the server! Try again later.")
    
    # Convertendo dados em lista:
    draa_parecer_atuarial_json <- jsonlite::fromJSON(httr::content(draa_parecer_atuarial, as = "text", encoding = "UTF-8"))
    
    # Empilhando dados (o padrao e 5000):
    dados_draa_parecer_atuarial <- dplyr::bind_rows(dados_draa_parecer_atuarial, draa_parecer_atuarial_json[["data"]])
    
    # Se alcancar o limite (5000), vai continuar a busca:
    continuar <- draa_parecer_atuarial_json[["count"]] == draa_parecer_atuarial_json[["limit"]]
    
    # Avança o offset para a proxima pagina:
    pagina <- pagina + draa_parecer_atuarial_json[["limit"]]
    
    Sys.sleep(1)
  }
  
  return(dados_draa_parecer_atuarial)
}
