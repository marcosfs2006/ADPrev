#' Extrai dados da Forma de Amortização do DRAA da API do CADPREV
#'
#' Função para a obtenção de dados relativos à Forma de Amortização do 
...
#' ao ponto de acesso \code{DRAA_FORMA_AMORTIZACAO} recomendamos utilizar os 
...
#'  para a consulta ao forma_amortizacao do DRAA.  
#' @return Um \code{data frame} contendo os dados requisitados.
#' @examples
#' \dontrun{ 
#' # Obtém dados da Forma de Amortização do DRAA feito pelos RPPS do RJ
#' draa_forma_amortizacao_RJ <- get_draa_forma_amortizacao(sg_uf="RJ") 
#' 
#' # Obtém dados da Forma de Amortização do DRAA feito pelo RPPS de Quatis - RJ
#' draa_forma_amortizacao_QuatisRJ <- get_draa_forma_amortizacao(nr_cnpj_entidade = "39560008000148")
#' }
#' @export
get_draa_forma_amortizacao <- function(...){
  
  consulta <- list(...) # Repassa parametros a api
  pagina <- 0
  dados_draa_forma_amortizacao <- data.frame()
  continuar <- TRUE
  
  while(continuar){
    
    # Acessando API:
    draa_forma_amortizacao <- httr::GET("https://apicadprev.trabalho.gov.br/DRAA_FORMA_AMORTIZACAO", 
                                            query = c(consulta, list(offset = pagina)))
    
    # Mensagem se o site estiver fora do ar ou der erro:
    httr::stop_for_status(draa_forma_amortizacao, task = "Connect to the server! Try again later.")
    
    # Convertendo dados em lista:
    draa_forma_amortizacao_json <- jsonlite::fromJSON(httr::content(draa_forma_amortizacao, as = "text", encoding = "UTF-8"))
    
    # Empilhando dados (o padrao e 5000):
    dados_draa_forma_amortizacao <- dplyr::bind_rows(dados_draa_forma_amortizacao, draa_forma_amortizacao_json[["data"]])
    
    # Se alcancar o limite (5000), vai continuar a busca:
    continuar <- draa_forma_amortizacao_json[["count"]] == draa_forma_amortizacao_json[["limit"]]
    
    # Avança o offset para a proxima pagina:
    pagina <- pagina + draa_forma_amortizacao_json[["limit"]]
    
    Sys.sleep(1)
  }
  
  return(dados_draa_forma_amortizacao)
}
