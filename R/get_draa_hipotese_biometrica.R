#' Extrai dados de Hipotese Biométrica do DRAA da API do CADPREV
#'
#' Função para a obtenção de dados relativos ao Hipotese Biométrica do 
...
#' ao ponto de acesso \code{DRAA_HIPOTESE_BIOMETRICA} recomendamos utilizar os 
...
#'  para a consulta ao hipotese_biometrica do DRAA.  
#' @return Um \code{data frame} contendo os dados requisitados.
#' @examples
#' \dontrun{ 
#' # Obtém dados da Hipótese Biométrica do DRAA feito pelos RPPS do RJ
#' draa_hipotese_biometrica_RJ <- get_draa_hipotese_biometrica(sg_uf="RJ") 
#' 
#' # Obtém dados da Hipótese Biométrica do DRAA feito pelo RPPS de Quatis - RJ
#' draa_hipotese_biometrica_QuatisRJ <- get_draa_hipotese_biometrica(nr_cnpj_entidade = "39560008000148")
#' }
#' @export
get_draa_hipotese_biometrica <- function(...){
  
  consulta <- list(...) # Repassa parametros a api
  pagina <- 0
  dados_draa_hipotese_biometrica <- data.frame()
  continuar <- TRUE
  
  while(continuar){
    
    # Acessando API:
    draa_hipotese_biometrica <- httr::GET("https://apicadprev.trabalho.gov.br/DRAA_HIPOTESE_BIOMETRICA", 
                                        query = c(consulta, list(offset = pagina)))
    
    # Mensagem se o site estiver fora do ar ou der erro:
    httr::stop_for_status(draa_hipotese_biometrica, task = "Connect to the server! Try again later.")
    
    # Convertendo dados em lista:
    draa_hipotese_biometrica_json <- jsonlite::fromJSON(httr::content(draa_hipotese_biometrica, as = "text", encoding = "UTF-8"))
    
    # Empilhando dados (o padrao e 5000):
    dados_draa_hipotese_biometrica <- dplyr::bind_rows(dados_draa_hipotese_biometrica, draa_hipotese_biometrica_json[["data"]])
    
    # Se alcancar o limite (5000), vai continuar a busca:
    continuar <- draa_hipotese_biometrica_json[["count"]] == draa_hipotese_biometrica_json[["limit"]]
    
    # Avança o offset para a proxima pagina:
    pagina <- pagina + draa_hipotese_biometrica_json[["limit"]]
    
    Sys.sleep(1)
  }
  
  return(dados_draa_hipotese_biometrica)
}
